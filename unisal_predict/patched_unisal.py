...
from unisal.unisal import utils
from unisal.unisal.models.cgru import ConvGRU
from unisal.unisal.models.MobileNetV2 import MobileNetV2, InvertedResidual
from unisal.unisal.model import DEFAULT_DEVICE
...
class UNISAL(BaseModel, utils.KwConfigClass):
    ...
    def forward(
        ...
        for idx, im_feat in enumerate(torch.unbind(feat_seq_1x, dim=1)):
            ...
            # Patched: if an odd-numbered factor in the shape causes a mismatch in dimensions, cut the last index
            if im_feat.shape[-2] > feat_seq_4x[idx].shape[-2]:
                im_feat = im_feat[:, :, :-1, :]
